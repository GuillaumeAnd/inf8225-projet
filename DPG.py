...
class Critic(nn.Module):  #pour les 2 critics
    ...
    def forward(self, x, u):
        ...
        z = torch.zeros_like(u)  #on enlève les actions des inputs de Critic
        x_cat = torch.cat([x, z], 1)
        ...
        return x1, x2  # On n'utilise que x1
...
class ReplayBuffer(object):  #on garde le Replay car essentielle pour décoréller
    def __init__(self):
        self.storage = []
        self.size = 1e6
        self.pos = 0
...
class  DPG(object): 
    ...
    def __init__(self, state_dim, action_dim, action_maxi):
        ...
        self.actor_target = Actor(state_dim, action_dim, action_maxi)  #inutile
        # c'est plus simple de le garder et de ne pas l'utiliser (par manque de temps)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, bruit_policy=0.2, bruit=0.5, policy_freq=2): 
      # hyperparamètres à modifier pour toruver l'optimale
        ...
        for i in range(iterations):
            ...
            next_action = (self.actor(next_state)).clamp(-self.action_maxi, self.action_maxi)


            #Calcul des estimations de Q
            target_Q1,a = self.critic_target(next_state, next_action)
            current_Q1,a = self.critic(state, action)
...
agent =  DPG(state_dim, action_dim, action_maxi)

###  DPG train + video
def  DPG_train(n_episodes=2000):
...
scores, avg_scores =  DPG_train()


import pandas as pd
res = pd.read_csv('DPG_res.txt', delimiter = ",", header=None)
res.head()
